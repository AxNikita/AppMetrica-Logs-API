...
class ErrorsSchema(BaseModel):
    """
    Ошибки.
    https://appmetrica.yandex.ru/docs/ru/mobile-api/logs/ref/errors
    """
    ...
    error_receive_datetime: Optional[datetime] = Field(
        default=None,
        description='Время получения сервером сообщения об ошибке в формате yyyy-mm-dd hh:mm:ss. Может отличаться от error_datetime из-за задержек в сети или отсутствия подключения у пользователя.'
    )
    error_receive_timestamp: Optional[int] = Field(
        default=None,
        description='Время получения сервером сообщения об ошибке в формате UNIX-time. Может отличаться от error_timestamp из-за задержек в сети или отсутствия подключения у пользователя.'
    )
    ...
    os_version: Optional[str] = Field(
        default=None,
        description='Версия операционной ��истемы на устройстве пользователя.'
    )
